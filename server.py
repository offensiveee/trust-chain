...
ADMIN_USERNAME = os.getenv("TRUSTCHAIN_ADMIN_USER", "")
ADMIN_PASSWORD = os.getenv("TRUSTCHAIN_ADMIN_PASS", "")

# Model API configuration
MODEL_DIR = BASE_DIR / "models"
MODEL_DIR.mkdir(exist_ok=True)
MODEL_FILES = {
    "lightgbm": MODEL_DIR / "lightgbm_fast.joblib",
...
# Numeric features use 0 as default (not empty string)
NUMERIC_FEATURE_COLUMNS = {"public_key_size", "pathlen"}
...
MODEL_RELEASE_REPO = os.getenv("TRUSTCHAIN_MODEL_RELEASE_REPO", "")
...
class TrustedChainHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_version(self):
        """Version and capability info — no auth required."""
        return self._send_json({
            "version": "1.0.0",
            "name": "TrustedChain",
            "model": "LightGBM",
    ...
    def _handle_predict(self):
        ...
        for col in FEATURE_COLUMNS:
            ...
            if value == "" or value is None:
                value = 0 if col in NUMERIC_FEATURE_COLUMNS else ""
        ...
        # Coerce all columns to numeric (required by LightGBM)
        ...
        try:
            ...
            # Compute risk metrics from class probabilities
            malicious_id = next((k for k, v in LABEL_MAP.items() if v == "malicious"), None)
            if probs is not None and malicious_id is not None and malicious_id < len(probs):
                risk_score = float(probs[malicious_id])
                confidence = float(max(probs))
            elif probs is not None:
                risk_score = float(probs[pred]) if label == "malicious" else 0.0
                confidence = float(max(probs))
            else:
                risk_score = 1.0 if label == "malicious" else 0.0
                confidence = 1.0
            ...
            response = {
                ...
                "risk_band": risk_band,
                "model": model_name,
                "features_received": len([v for v in features.values() if v != "" and v is not None]),
            ...
            if probs is not None:
                response["probabilities"] = {
                    LABEL_MAP.get(i, str(i)): round(float(p), 4) for i, p in enumerate(probs)
                }
