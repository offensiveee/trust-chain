...
def train_model(X, y):
    ...
    label_names = [str(i) for i in range(y_pred_proba.shape[1])]

    print(f"\nAccuracy  : {accuracy_score(y_test, y_pred_labels):.4f}")
    print(f"Precision : {precision_score(y_test, y_pred_labels, average='macro', zero_division=0):.4f}  (macro)")
    print(f"Recall    : {recall_score(y_test, y_pred_labels, average='macro', zero_division=0):.4f}  (macro)")
    print(f"F1-Score  : {f1_score(y_test, y_pred_labels, average='macro', zero_division=0):.4f}  (macro)")
...
def save_model(model):
    """Save trained model"""
    model_path = MODEL_DIR / "lightgbm_fast.joblib"
...
def main():
    ...
    print("Run evaluation: python -m src.evaluate")
    print("=" * 60)
